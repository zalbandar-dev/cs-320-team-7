...
export default function SpotDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      try {
        const res = await fetch(`/api/getSpotReviews/${params.id}`);
        const data = await res.json();
        if (data.success) {
          setReviewAvg(data.data.average);
  ...
  const hours =
    startTime && endTime
      ...
      : 0;
  const estimatedPrice = spot
    ? parseFloat((hours * spot.hourly_rate).toFixed(2))
    : 0;

  async function handleBook() {
    setBookingError("");
    if (!startTime || !endTime)
      return setBookingError("Please select start and end times.");
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            ${estimatedPrice.toFixed(2)} total
          </p>
          <div
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <h1
              style={{ fontSize: "26px", fontWeight: 700, margin: "0 0 6px" }}
            >
              {spot.address}
            </h1>

            <div style={{ marginBottom: "16px" }}>
              <StarDisplay average={reviewAvg} count={reviewCount} />
            </div>
                  ...
                  {hours > 0 && (
                    ...
                    >
                      <span style={{ fontSize: "13px", color: "#0369a1" }}>
                        {hours.toFixed(1)} hrs × ${spot.hourly_rate.toFixed(2)}
                      </span>
                      <span
                        style={{
                          fontSize: "15px",
